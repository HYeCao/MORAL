...
class AdversarialDynamics(object):
    ...
    def _predict_q(self, state, action, next_state, next_value_target, next_value, mean_reward, penalty):
        ...
        # do reward normal
        reward_ = reward_ - penalty
        # bellman Q value calculation
    ...
    def _predict_adv_q(self, state, action, test_mode):
        ...
        # define the differential factor
        penalty = np.mean(ensemble_std.cpu().numpy()) if ensemble_std is not None else 0
        penalty = penalty * 0.5
        ...
        #  q-value prediction with penalty
        q_value_target, q_value, reward_candi, done_prob_candi \
            = self._predict_q(state, action, next_state_candi, next_value_target, next_value, mean_reward, penalty)
